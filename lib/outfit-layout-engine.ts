...
export function calculateAutoPosition(
  ...
  const positionX = Math.max(0.2, Math.min(0.8, 0.5 + (itemCountInCategory % 2 === 0 ? horizontalSpread : -horizontalSpread)))
 ...
 */
export function normalizePosition(pixelX: number, pixelY: number): { x: number; y: number } {
  return {
 ...
 */
export function denormalizePosition(normalizedX: number, normalizedY: number): { x: number; y: number } {
  return {
