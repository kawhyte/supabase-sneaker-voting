 ...
 */
export const CARD_LAYOUT_TEMPLATE: Record<
	string,
	{
		x: number;
		y: number;
...
> = {
	...
	shoes: {
		x: 0.5, // 200px from left
		y: 0.6, // 288px from top
		width: 0.3, // 120px wide
		height: 0.3, // 144px tall
		zIndex: 10,
	},
 ...
 */
export function selectItemsForCard(items: OutfitItem[]): OutfitItem[] {
	...
	const remaining = items.filter(
		(i) => !tier1.includes(i) && !tier2.includes(i)
	);
	...
	const selected = [
		...
		...remaining.slice(
			0,
			Math.max(0, MAX_CARD_ITEMS - tier1.length - tier2.length)
		),
	];
...
export function applyCardLayout(items: OutfitItem[]): OutfitItem[] {
	return items.map((item) => {
		...
		const template =
			CARD_LAYOUT_TEMPLATE[category] || CARD_LAYOUT_TEMPLATE.other;
 ...
 */
export function prepareOutfitForCard(outfit: OutfitWithItems): {
	items: OutfitItem[];
	...
	const sortedItems = layoutItems.sort(
...
export function validateCardLayout(items: OutfitItem[]): boolean {
	...
	return items.every(
		(item) =>
			...
			item.display_width <= 1 &&
			item.display_height > 0 &&
			item.display_height <= 1
	);
}
