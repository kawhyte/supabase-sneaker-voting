...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: item, error: fetchError } = await supabase
      .from('items')
      .select('id, product_url, retail_price, sale_price, lowest_price_seen, target_price, target_alert_sent_at, brand, model, user_id, auto_price_tracking_enabled, sku, size_tried')
      .eq('id', itemId)
      .eq('user_id', user.id)
      .single()
    ...
    // Check if target price reached — use persistent latch, not a time window
    let newTargetAlertSentAt = item.target_alert_sent_at ?? null
    if (item.target_price) {
      if (result.price <= item.target_price) {
        ...
        // else: alert already sent this pricing period, stay silent
      } else {
        // Price rose above target — reset latch so we alert again next time it drops
