...
const SneakerCard = ({ sneaker, onDelete, onVote, showElement }) => {
	...
	return (
		<div>
			<div className='w-full  max-w-5xl flex flex-col  container '>
				<div className='relative    '>
					<div className=''>
						<Carousel className=' bg-gray-800'>
													...
													<CardHeader className='bg-white'>
														<span className=' font-semibold max-w-xl flex aspect-square items-center justify-center '>
															<img
																...
																//className="w-full object-cover h-80 lg:h-1\3"
																src={item?.image_link}
																alt='product image'
																loading='lazy'
															/>
							...
							{/*Card text area*/}
							<div>
								<CardContent className='bg-gray-800 pt-5 text-[0.85rem] '>
									<h5 className=' font-mono line-clamp-none lg:line-clamp-1 flex flex-col normal-case text-start  text-[1.3rem] sm:text-[0.89rem] tracking-[-0.01em] leading-[1.33] font-semibold'>
										{sneaker.name}
									</h5>
									<p className='tracking-wide  title-font font-medium text-gray-400 mb-1 mt-3 font-mono'>
										Brand: {sneaker.brand_id?.name}
									</p>
									<div className='tracking-wide  title-font font-medium text-gray-400 my-2 font-mono'>
										<div className=''>
											<span>
												Release Date:{" "}
												{format(new Date(sneaker.release_date), "MMMM dd,yyyy")}
											</span>
											<span className=' rounded-xl border py-1 px-2 text-blue-600  bg-blue-200 font-mono leading-[1.2] text-[.8em] ml-2 mb-2'>
												{databaseDate > todayDate ? "Upcoming" : "Released"}
											</span>
										</div>
									</div>

									<p className='tracking-wide  title-font font-medium text-gray-400 my-1 font-mono mb-6'>
										Retail: {sneaker.price < 10 ? "TBD" : `$${sneaker.price}`}-{" "}
										{sneaker.style} - {sneaker.id}
									</p>
									{!showElement && (
										<div className="border-t ">
											<h2 className='sm:tracking-wide  xl:hidden    sm:text-[0.82rem] title-font font-medium text-white mb-1 mt-5  font-mono grid justify-items-center'>
												Select one:
											</h2>

											<div className='flex flex-row justify-evenly mx-12 sm:mx-3 xl:justify-between  rounded-xl  max-w-xl  mb-8 xl:mt-7 sm:mt-2 sm:mb-7 '>
												<h2 className='sm:tracking-wide hidden sm:ml-2  sm:text-[0.83rem] title-font text-start xl:flex align-middle justify-start items-center font-medium text-white my-1  font-mono'>
													Select one:
												</h2>
												<div className=' flex justify-between  '>
													<div className='relative group flex flex-col items-center align-middle justify-center'>
														...
														<span
															className={`absolute top-8 md:scale-0 transition-all  mt-6 text-center rounded bg-gray-800  text-[0.67rem] text-xs text-white md:group-hover:scale-100 ${
																vote === "1" ? "scale-100" : ""
															}`}>
															Love it!
														</span>
									...
									<div className='flex justify-between font-mono  mt-16 text-gray-400 text-[0.7rem]  '>
										{/* Create listing button*/}
										<Link className='' href={"/sneakers/create/"}>
											<Button
												variant='outline'
												className='hover:bg-gray-800  group relative'>
												<MdOutlineCreate className=' h-5 w-5' />
												<p className='sm:opacity-0 sm:group-hover:opacity-100 duration-300 absolute -inset-x-6 bottom-10 justify-center items-end text-[0.7rem]'>
													Create New{" "}
												</p>
											</Button>
										</Link>
					...
					<div className=' absolute top-3 right-3 flex flex-row justify-end items-center text-base'>
						...
						{vote === "4" && (
							<RatingCard text={"I like it 😀"} bg={"bg-indigo-200"} />
						)}
