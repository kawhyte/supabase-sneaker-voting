...
export default function SneakerCard({
	...
	console.log("Date Comapre ",result)
						...
						{!showElement && (
							<div className="border-b-2">
								...
								<CardContent className='flex flex-col justify-between   text-sm tracking-wide  title-font font-medium text-gray-400 font-mono '>
									...
									{/* <Badge className='bg-yellow-200 '>{sneaker.brand_id?.name}</Badge>
									<Badge className='bg-green-200 '> Release:{" "}{format(new Date(sneaker.release_date), "MMM dd,yyyy")}</Badge>
									<Badge className='bg-indigo-200 '>Retail:{" "}{sneaker.price < 10 ? "TBD" : `$${sneaker.price}`}</Badge> */}
									</div>
